...
def chunks(l,n):
    ...
    #it will add a # to the last group to make them all ==n
    chunked_list=[l[i:i + n] for i in range(0, len(l), n)]
    to_add=n-len(chunked_list[-1])
    chunked_list[-1]+=('#'*to_add)
    return chunked_list
